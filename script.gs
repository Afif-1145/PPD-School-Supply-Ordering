...
function handleFindUser(email) {
  ...
  try {
    ...
    if (lastRow < 2) {
      Logger.log('FindUser end (ms): ' + (new Date().getTime() - start));
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (sheetEmail === checkEmail) {
        const user = { name: data[i][0], email: data[i][1], hint: data[i][6] || '' };
        Logger.log('FindUser end (ms): ' + (new Date().getTime() - start));
    ...
    Logger.log('FindUser Error: ' + error.toString());
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (sheetEmail === checkEmail) {
        ...
        Logger.log('Password reset for: ' + email + ' (ms: ' + (new Date().getTime() - start) + ')');
        return ContentService.createTextOutput(JSON.stringify({
          success: true,
    ...
    Logger.log('ResetPassword end (ms): ' + (new Date().getTime() - start));
    return ContentService.createTextOutput(JSON.stringify({
      success: false,
      message: 'Pengguna tidak dijumpai'
    })).setMimeType(ContentService.MimeType.JSON);
